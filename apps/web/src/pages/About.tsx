    ...
    <section>
      <Container>
        <div className="max-w-3xl mx-auto text-center mb-10">
          <h2 className="text-3xl font-bold text-gw-black mb-4">About Gadget Wallet</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-14">
          {[
            ...
            <div key={item.title} className="bg-white border border-gw-border rounded-category p-6 text-center hover:-translate-y-1 hover:shadow-gw-md transition-all duration-300">
              <item.icon className="w-10 h-10 text-gw-red mx-auto mb-4" />
              <h3 className="font-semibold text-gw-black mb-1">{item.title}</h3>
              <p className="text-sm text-gw-gray-500">{item.desc}</p>
            </div>
          ))}
        </div>

        <div className="bg-white border border-gw-border rounded-[24px] p-8 max-w-4xl mx-auto">
